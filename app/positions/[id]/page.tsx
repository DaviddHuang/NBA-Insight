...
const formatSlugToTitle = (slug: string): string => {
  const formattedTitle = slug
    ...
    .join(" ");
  return positionMap[formattedTitle] || formattedTitle;
};

const PlayerPage = async ({ params }: PageProps) => {
  const posName = formatSlugToTitle(params.id);
  ...
  try {
      ...
      return (
        <div>
          ...
          <div className="customFontClass">
            No players found for position "{posName}".
          </div>
        </div>
      );
    }
    ...
    return (
      <div>
        <Navbar />
        <div className="customFontClass">
          Failed to load players for team "{posName}". Please try again later.
        </div>
      </div>
    );
  }
